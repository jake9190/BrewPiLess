...
The retained availability state is published as `online` or `offline` on `<Base Path>/status`. With **Whole as JSON**, the JSON payload is published on `<Base Path>/json`.
