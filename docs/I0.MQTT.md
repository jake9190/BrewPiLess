...
The retained availability state is published as `online` or `offline` on a `status` topic. With **Individual Topic**, this is `<Base Path>/status`. With **Whole as JSON**, it is a sibling of the JSON topic: a base path of `/base/data` uses `/base/status`.
